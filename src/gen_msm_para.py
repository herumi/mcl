...
static const CYBOZU_ALIGN(64) uint64_t g_vmask_[] = {{ {"g_mask, "*8}}};
static const uint64_t g_rp = {hex(mont.rp)};
static const CYBOZU_ALIGN(64) uint64_t g_vrp_[] = {{ {"g_rp, "*8}}};''')
  for i in range(len(vp)):
    print(f'static const uint64_t g_p{i} = {hex(vp[i])};')
  print('static const CYBOZU_ALIGN(64) uint64_t g_vpN_[] = {')
  for i in range(len(vp)):
    print('\t' + (f'g_p{i}, '*8).strip())
  print('};')
