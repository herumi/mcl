...
/*
	register the p-fixed LLVM functions of BLS12-381 (mcl_c5_*, generated by
	src/gen.py, the prototypes and set_llvm_c5_* in llvm_proto.hpp) to the A_
	slots of op if p is the p or r of BLS12-381.
...
{
	if (!op.isMont) return;
	if (isSamePrime(op, mcl_c5_fp_p, 384)) {
		set_llvm_c5_fp(op);
		// Fp2 = Fp[i]/(i^2 + 1), xi = 1 + i and sizeof(Fp) == 48 (the offset of the second component is fixed)
		if (op.u == 1 && op.xi_a == 1 && op.maxN * UnitBitSize == 384) {
			set_llvm_c5_fp2(op);
		}
