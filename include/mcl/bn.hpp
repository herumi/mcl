...
{
	switch (type) {
	case 0: return CurveFp254BNb;
	case 1: return CurveFp382_1;
	case 2: return CurveFp382_2;
	case 3: return CurveFp462;
	case 4: return CurveSNARK1;
	default:
...
struct BNT {
	...
	*/
	static void mul_024(Fp12& z, const Fp6& x)
	{
		...
		Fp2Dbl D0, D2, D4;
		Fp2Dbl S1;
		Fp2Dbl_mulOpt(D0, z0, a);
		Fp2Dbl_mulOpt(D2, z2, b);
		Fp2Dbl_mulOpt(D4, z4, c);
		Fp2::add(t2, z0, z4);
		Fp2::add(t1, z0, z2);
		Fp2::add(s0, z1, z3);
		s0 += z5;
		// For z.a.a = z0.
		Fp2Dbl_mulOpt(S1, z1, b);
		Fp2Dbl::add(T3, S1, D4);
		Fp2Dbl::mul_xi(T4, T3);
		T4 += D0;
		Fp2Dbl::mod(z0, T4);
		// For z.a.b = z1.
		Fp2Dbl_mulOpt(T3, z5, c);
		S1 += T3;
		T3 += D2;
		Fp2Dbl::mul_xi(T4, T3);
		Fp2Dbl_mulOpt(T3, z1, a);
		S1 += T3;
		T4 += T3;
		Fp2Dbl::mod(z1, T4);
		// For z.a.c = z2.
		...
		T3 -= D4;
		Fp2Dbl::mul_xi(T4, T3);
		Fp2Dbl_mulOpt(T3, z3, a);
		S1 += T3;
		T4 += T3;
		Fp2Dbl::mod(z3, T4);
		// For z.b.b = z4.
		Fp2Dbl_mulOpt(T3, z5, b);
		S1 += T3;
		Fp2Dbl::mul_xi(T4, T3);
		Fp2::add(t0, a, c);
